...
let color = ['darkturquoise', 'darkviolet', 'fuchsia', 'gold', 'green', 'deeppink'];
let startYear = 2100;
let endYear = 1900;
let parsedData;
...
function fetchAndParseCSV() {
  return new Promise((resolve, reject) => {
    ...
    xhr.open('GET', 'timelineTest2.csv', true); // Use true for asynchronous request
    xhr.onload = function() {
      if (xhr.status === 200) {
...
function draw() {
  canvas.width = 4000;//window.innerWidth - 50;
  ...
  parsedData.forEach(item => {
    ...
    ctx.fillStyle = color[type.length % 5];
