...
//    if(empty($name_err) && empty($address_err) && empty($salary_err)){
        ...
        $stmt = $pdo->prepare("UPDATE oequipments SET EN=?,TR=?,help_TR=?,help_EN=? WHERE id=?");
        ...
        if(!$stmt->execute([ $EN,$TR,$help_TR,$help_EN,$id  ])) {
                ...
                $stmt = null;
                header("location: oequipments-index.php");
            }
} else {
    ...
    if(isset($_GET["id"]) && !empty(trim($_GET["id"]))){
        ...
        $sql = "SELECT * FROM oequipments WHERE id = ?";
            ...
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <a href="oequipments-index.php" class="btn btn-secondary">Cancel</a>
                    </form>
                </div>
            </div>
        </div>
